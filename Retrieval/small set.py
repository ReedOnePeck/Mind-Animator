...
model = Semantic_Network(in_dim=4500, out_dim=19 * 768, h=512, n_blocks=2, norm_type='ln', act_first=False)
...
test_set_CLIP_emb = np.load('/nfs/nica-datashop/CC2017_for_video_reconstruction/stimuli_frames/Test/Test_text_embeddings_CLIP_512.npy')

semantic = model(torch.tensor(test_src)).cpu().detach().numpy()

data = test_set_CLIP_emb
queries = semantic
labels = np.arange(1200)

top_1_acc = top_k_accuracy(data, queries,labels, k=10)
top_5_acc = top_k_accuracy(data, queries, labels, k=50)

print(f"Top-10 Accuracy: {top_1_acc:.4f}")
#print(f"Top-50 Accuracy: {top_5_acc:.4f}")
